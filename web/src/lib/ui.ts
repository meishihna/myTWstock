...
export const MAP_PAGE = {
  nav: "題材",
  title: "投資題材",
  lead: "策展投資題材(如 AI 伺服器、CPO、矽光子):熱力圖看當日題材漲跌,點題材進入供應鏈與成分股。官方產業別分類請看導覽「產業」。",
  tabHeatmap: "熱力圖",
  tabIndustry: "主題",
...
export const TRADES_PAGE = {
  ...
  /**
   * 🔴 這句話**不得陳述登入狀態或資料有無** —— 它是佔位文字,而佔位文字會被
   *    真實資料推翻。原文是「登入後手動輸入你的交易…」,結果一個已登入、
   *    有 48 筆交易的使用者看到它 —— **那句話當下是假的**。
   *    (同「尚無持股」那個病:佔位陳述了可驗證的事實,而且與「算出來真的沒有」
   *     在畫面上長得一模一樣,分不清是 bug 還是未實作。)
   * 改成【機制描述】:機制不會因為登入或有沒有資料而變成假的。
   */
  emptyCta: "交易由你自行輸入;系統以公開股價計算損益與績效",
