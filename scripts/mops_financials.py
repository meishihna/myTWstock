...
def _t163_parse_row_securities(norm_cols: list[str], row: pd.Series) -> dict[str, float | None]:
    ...
    out["Revenue"] = _t163_cell(row, rev_i, True)
    # 證券業「支出及費用」對應 G&A（與 insurance「營業費用」等效）
    out["General & Admin Exp"] = _t163_cell(
        row, _t163_col_substring(norm_cols, "支出及費用"), True
    )
...
_T163_CACHE_TICKER_SET: frozenset[str] | None = None


def _all_tickers_in_t163_caches() -> frozenset[str]:
    """全市場 t163 快取（sb04／sb06／sb20 等）內出現過的代號集合；程序內快取一次。"""
    global _T163_CACHE_TICKER_SET
    if _T163_CACHE_TICKER_SET is not None:
        return _T163_CACHE_TICKER_SET
    s: set[str] = set()
    if os.path.isdir(MOPS_CACHE_DIR):
        for path in glob.glob(os.path.join(MOPS_CACHE_DIR, "*.json")):
            try:
            ...
            for tk in bt:
                if isinstance(tk, str) and tk.isdigit():
                    s.add(str(int(tk)))
    _T163_CACHE_TICKER_SET = frozenset(s)
    return _T163_CACHE_TICKER_SET


def ticker_in_any_t163_cache(ticker: str) -> bool:
    """若代號出現於任一向全市場 t163 快取（by_ticker）則為 True。

    興櫃偵測：三表皆無命中且具半年報特徵時可標為 emerging。
    """
    tid = _norm_ticker_cell(ticker)
    if not tid.isdigit():
        return False
    tid = str(int(tid))
    return tid in _all_tickers_in_t163_caches()
