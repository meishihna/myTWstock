...
description: 產生/更新個股報告頁「營收異動原因」的一句話 why(深度層)。由 Claude Code 讀近期新聞頭條歸納,無需 API key。
user-invocable: true
---

# Revenue Reasons（營收異動原因）

**個股報告頁**顯示**為什麼**——營收突然增/減的原因(轉型、接單、漲價、需求…)。
(自選股總覽只留清爽的「月營收異動」旗標;深入的原因放單檔報告頁,空間充裕。)
...
- **純文字**(報告頁直接顯示 why):**勿**加 `[[ ]]` wikilink、勿用 Markdown。
...
- **schema 契約**:`web/src/pages/report/[ticker].astro`(build 時讀 `notes[ticker]` 的 `heads` 陣列與 `why` 字串);勿更動鍵名。
